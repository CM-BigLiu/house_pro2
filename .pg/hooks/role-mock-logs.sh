#!/usr/bin/env bash
# role lifecycle action: mock logs
# 对应 project.yaml: environments.dev-mock.roles.mock.actions.logs
...
if [[ -f "$LOG_DIR/mock.log" ]]; then
    tail -n "$LINES" "$LOG_DIR/mock.log"
else
    echo "no mock log at $LOG_DIR/mock.log"
fi
