...
>
> 方法：调用 Task tool，将 `subagent_type` 设为 `explore`。在 prompt 中提示 explore agent **优先使用 codegraph 系列工具**（codegraph_context/codegraph_search/codegraph_files 等），而非直接读取文件。仅在子 agent 返回结果不足时，再由你使用 codegraph 工具直接查询或补充。
>
> **为什么这是硬约束：**
